...
`;
  ...
  // Menu management
  updateWorldState: (worldSelected) => ipcRenderer.invoke('update-world-state', worldSelected),
  getWorldState: () => ipcRenderer.invoke('get-world-state'),
  
  // Listen for navigation events from menu
  onNavigate: (callback) => {
    window.addEventListener('electron-navigate', callback);
    return () => window.removeEventListener('electron-navigate', callback);
  }
});